...
mod tests {
    ...
    fn test_sandbox_network_permissions() {
        let perms = PluginPermissions {
            network: Some(NetworkPermissions {
                domains: Some(vec!["example.com".to_string(), "api.github.com".to_string()]),
            }),
            ..Default::default()
        };
    ...
    fn test_sandbox_gateway_permissions() {
        let perms = PluginPermissions {
            gateway: Some(GatewayPermissions {
                methods: Some(vec!["sessions:list".to_string(), "agents:invoke".to_string()]),
            }),
            ..Default::default()
        };
    ...
    fn test_sandbox_manager_check_permission() {
        ...
        assert!(manager.check_permission(
            "test-plugin", 
            &PermissionCheck::Domain("example.com")
        ));
        assert!(!manager.check_permission(
            "test-plugin", 
            &PermissionCheck::Domain("evil.com")
        ));
    }
