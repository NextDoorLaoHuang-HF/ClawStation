...
use crate::plugins::sandbox::{PermissionCheck, PluginSandbox, SandboxManager};
...
impl PluginManager {
    ...
    pub async fn init(&mut self) -> Result<(), String> {
        ...
        PluginLoader::ensure_plugins_dir().map_err(|e| e.to_string())?;

        // Load installed plugins list
        self.installed = PluginLoader::load_installed_list().map_err(|e| e.to_string())?;
    ...
    pub async fn load_all(&mut self) -> Result<(), String> {
        let plugins = PluginLoader::list_installed().map_err(|e| e.to_string())?;
    ...
    pub async fn install(&mut self, source: &str) -> Result<PluginInfo, String> {
        ...
        // Validate the plugin
        let errors = PluginLoader::validate_plugin(&source_path).map_err(|e| e.to_string())?;
        ...
        let plugin = PluginLoader::load_plugin(&source_path).map_err(|e| e.to_string())?;
        ...
        let installed_plugin = PluginLoader::load_plugin(&target_dir).map_err(|e| e.to_string())?;
        ...
        // Save installed list
        PluginLoader::save_installed_list(&self.installed).map_err(|e| e.to_string())?;
    ...
    pub async fn reload(&mut self, id: &str) -> Result<(), String> {
        ...
        // Load fresh
        let plugin = PluginLoader::load_plugin(&plugin_path).map_err(|e| e.to_string())?;
